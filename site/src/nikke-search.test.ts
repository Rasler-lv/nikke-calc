...
describe('rankOf', () => {
  const crown = buildIndex(meta('Crown', { elementCode: '수냉' }));
  ...
  it('걸리지 않으면 NO_MATCH', () => {
    expect(rankOf('Alice', crown)).toBe(NO_MATCH);
  });
});
...
describe('filterByQuery', () => {
  ...
  it('구분자를 무시해 「Rapi레드」가 «Rapi : Red Hood»를 잡는다', () => {
    expect(pick('Rapi레드')).toEqual(['Rapi : Red Hood']);
  });

  it('속성·무기로도 걸리지만 이름보다 뒤로 밀린다', () => {
    expect(pick('SR')).toEqual(['목단']);
    expect(pick('Fire Code')).toEqual(['Rapi : Red Hood']);
  });
...
describe('별칭 검색', () => {
  ...
  it('별칭이 없는 캐릭터는 종전 그대로다', () => {
    expect(rankOf('세이렌', buildIndex(meta('Crown')))).toBe(NO_MATCH);
  });
});
