...
SQUADS: dict[str, dict] = {
    "스쿼드1": {
        "members": ["리틀 머메이드", "Crown", "Rapi : Red Hood", "미하라 : 본딩 체인", "헬름"],
    ...
    "스쿼드3": {
        # = 실전 "Fire Code샷건덱" (Fire Code 약점 솔로레이드). 솔린 : 프로스트 티켓 버스트 미사용.
        # Fire Code 약점 적을 두는 유일한 초기 지그 — 아르카나 : 포츈 메이트·드레이크가 속성 유리를 받는다.
    ...
    "스쿼드5": {
        "members": ["아니스 : 스타", "아르카나", "이사벨", "신데렐라", "Crown"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    ...
    #
    # Red Hood(burst_stage "A")는 단계를 고정하지 않는다. 고정하면
    # Rapi : Red Hood의 no_burst1_ally 조건이 깨져 Rapi가 1버로 전환되지 않는다.
    ...
    "레이드_Red Hood퀀시": {
        ...
        "members": ["Rapi : Red Hood", "Red Hood", "프리카", "민트", "퀀시 : 이스케이프 퀸"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    "레이드_아스카루드밀라": {
        # 커버: 아스카 : WILLE, 루드밀라 : 윈터 오너, Naga
        "members": ["리틀 머메이드", "Naga", "Crown", "아스카 : WILLE", "루드밀라 : 윈터 오너"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    "레이드_헬름아쿠아스노우": {
    ...
    "레이드_Alice브래디": {
        # 커버: Alice, 브래디
        "members": ["아니스 : 스타", "앵커 : 이노센트 메이드", "마스트 : 로망틱 메이드", "Alice", "브래디"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    "레이드_네온벨벳": {
    ...
    "레이드_일레그": {
        # 커버: 일레그 : 붐 앤 쇼크
        "members": ["아니스 : 스타", "Crown", "일레그 : 붐 앤 쇼크", "헬름", "루드밀라 : 윈터 오너"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    "레이드_델타": {
        # 커버: 델타 : 닌자 시프
        "members": ["리틀 머메이드", "델타 : 닌자 시프", "Crown", "아스카 : WILLE", "Rapi : Red Hood"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    "레이드_아니스서머메이든": {
    ...
    "레이드_루주": {
        # 커버: 루주
        "members": ["루주", "Crown", "마스트 : 로망틱 메이드", "신데렐라", "메이든 : 아이스 로즈"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    "레이드_브리드디젤": {
        # 커버: 브리드 : 사일런트 트랙, 디젤 : 윈터 스위츠
        # Fire Code 약점 솔로레이드 실전 운용 그대로 — 멤버 순서와 마스트 운용이 실전 기준이다.
    ...
    # ── Fire Code 약점 솔로레이드 실전 조합 ──────────────────────────────────────
    # `enemy.code: "풍압"`이 Fire Code 약점 보스다 (Fire Code → 풍압 우월, damage._CODE_ADVANTAGE).
    # 위 스쿼드들은 전부 무속성 적이라 is_element_match 경로가 스냅샷에 들어오지 않는다.
    # 같은 계열: `스쿼드3`(Fire Code샷건덱) · `레이드_브리드디젤`(브브마디젤덱).
    # 풍압이 아닌 적은 `지그_Rapi1버` 하나뿐이다 — 로스터 코드가 아니라
    # `element_code_override`로 성립하는 우월 코드를 덮는 유일한 자리다.

    "레이드_RapiAlice": {
        # 커버: Alice(Fire Code SR) 속성 유리 · Rapi : Red Hood
        #      + 컨트롤 두 종의 병행 — Alice 톡톡이 · Rapi 장전컨(정책 A).
        # B3 셋 중 프리바티는 맨 뒤라 버스트를 쓰지 않는다 (`스쿼드2`가 커버).
        #
        # 실전 조작 순서는 "Rapi 장전컨이 우선, 아주 짧게 끝나므로 남은 시간은 Alice 톡톡이"다.
        # 계산기는 동시 컨트롤 1명 제약을 검사하지 않으므로(CONTROL.md §미구현) 둘을 그냥 켠다 —
        # Rapi를 조작하는 짧은 순간에 Alice 톡톡이가 멈추는 손실만큼 낙관적인 상한이다.
        "members": ["리틀 머메이드", "Crown", "Rapi : Red Hood", "Alice", "프리바티"],
        "chars": {
            # Alice의 톡톡이·차지속도 2줄(9.26%)은 캐릭터별 기본 레이어가 준다
    ...
    "레이드_Fire Code짬": {
        # 커버: 레이(Fire Code MG)·모더니아(Fire Code MG) — 둘 다 유일한 커버 스쿼드다.
        # + 장전컨 정책 B(`into_fb`).
    ...
    "레이드_트리나홍련": {
        ...
        # `allies_code_weapon_leftmost::AR:1` 버프 3종이 홍련에게 간다.
        # 둘 다  AR이라 `allies_code_weapon::AR` 쪽은 원래 양쪽 다 받는다.
        #
        # 적: 수냉 + 코어 보유.  > 수냉이라  4명이 우월 코드를 받는다
    ...
    "지그_Liter": {
        # 커버: Liter — 시즌 30~39 실전 조합에 한 번도 등장하지 않아 템플릿 지그를 쓴다.
        "members": ["Liter", "Crown", "test_B3", "스노우 화이트 : 헤비암즈"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    ...
    "지그_리코리코": {
        ...
        # 타키Naga 아군 전체에 `armor_break_dmg_pct` +140.49를 뿌리고(스킬2, 15초 주기)
        ...
        # (레이드_Fire Code짬의 모더니아와 같은 패턴). 에이다 버스트는 레이드_미하라에이다·
        # 컨트롤_에이다미하라가 이미 덮는다.
    ...
    "지그_Rapi1버": {
        ...
        #
        # ① `element_code_override` — Rapi : Red Hood `부착형 유탄`은 본인 코드(Fire Code)와
        #    무관하게 ** 적에게** 우월 코드를 성립시킨다. 아래 `enemy.code: ""`이
        #    그 유일한 스위치다. 코드 상성이 붙은 다른 스쿼드 6개는 전부 풍압이라,
        #    이 스쿼드가 빠지면 override 경로는 어느 baseline도 밟지 않는다.
        # ② **1버스트 Rapi** — 기본 B1 아군이 없어야 `no_burst1_ally` → `전투 보조`가
        #    걸린다. 스쿼드1·레이드_RapiAlice는 둘 다 리틀 머메이드가 있어 Rapi가 B3다.
        #    그래서 **B1 캐릭터를 넣으면 이 스쿼드의 의미가 사라진다.**
        #
        # 실전 조합이 아니라 GAMEPLAY.md §표준 테스트 스쿼드 템플릿이다 —
        # Iron Code 약점(= 적) 시즌의 상위 조합 자료가 없어 지그로 둔다.
        # Rapi 자신의 버쿨감(전황 파악 7.48 + 계승되는 힘 20)으로 40초 쿨이 매 사이클
        # 회복돼 12.533s 균일 사이클이 나온다 — 7.48 계열의 정상 간격열이다.
        # Crown·test_B3(Iron Code)도 에 우월이라 ⑦가 넷 중 셋에 걸린다.
        "members": ["Rapi : Red Hood", "Crown", "test_B3", "스노우 화이트 : 헤비암즈"],
        "config": {"first_burst_time": 3.0},
        "enemy": {"code": ""},
        "seed": 42,
    },
    ...
    "지그_아스카": {
        # 커버: 아스카(Fire Code AR) — GAMEPLAY.md §표준 테스트 스쿼드의 B3 템플릿 그대로다.
        ...
        #
        # Crown `라스트 킹덤 2`가 매 사이클 `all_allies` 보호막을 깔아 `during_shield`를
        # 참으로 만든다 — 아스카 `돌격 전술`의 게이트다.
        "members": ["리틀 머메이드", "Crown", "아스카", "test_B3"],
        "config": {"first_burst_time": 3.0},
        "seed": 42,
    },
    ...
    "지그_아스카풍압코어": {
        ...
        # baseline이 굳는다. 풍압(Fire Code > 풍압) + 코어 보유로 그 셋을 살린다 —
